              ...
              return (
                <div key={debt.id} className="card" style={{ background: 'rgba(59,130,246,0.05)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span className={`badge badge-${debt.type === 'bad' ? 'danger' : 'success'}`}>{debt.type === 'bad' ? 'Плохой' : 'Хороший'}</span>
                  </div>
                  <div style={{ fontSize: 13, color: 'var(--subtext)', marginTop: 4 }}>
                    Баланс: {debt.balance.toLocaleString()} | Ставка: {debt.interestRate}% | Срок: {payoff.years} лет | Переплата: {payoff.totalInterest.toLocaleString()}
                  </div>
                </div>
              );
            })}
